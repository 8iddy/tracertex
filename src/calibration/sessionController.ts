...
import { getActiveLocalUserId, getLatestProfile, getSessions, saveEventBatch, saveProfile, saveSession } from "../storage/indexedDb";
...
export async function finalizeSession(active: ActiveSession, finalDocument: string, burstThresholdMs: number, sync: boolean): Promise<WritingSession> {
  ...
  const profile = { ...aggregateWriterProfile(sessions, await getLatestProfile()), id: crypto.randomUUID(), userId };
