import { ArrowRight, CheckCircle2, FileText, Keyboard, Layers3, Sparkles } from "lucide-react";
...
export function OnboardingCompletePage() {
  ...
  return <main className="onboarding-shell"><section className="onboarding-card completion-card">
    <span className="success-kicker"><CheckCircle2 size={15} />Core calibration complete</span>
    ...
    <div className="onboarding-metrics"><div><Layers3 size={17} /><span>Sessions observed<strong>{profile?.sampleSessions ?? 0}</strong></span></div><div><FileText size={17} /><span>Words observed<strong>{profile?.sampleWords.toLocaleString() ?? 0}</strong></span></div><div><Keyboard size={17} /><span>Writing events observed<strong>{profile?.sampleEvents.toLocaleString() ?? 0}</strong></span></div><div><CheckCircle2 size={17} /><span>Profile maturity<strong>{profile?.confidence.overall ?? 0}%</strong></span></div></div>
    <p>Your profile will keep improving as you record more genuine writing.</p>
    {error && <p className="error-text" role="alert">{error}</p>}
    <div className="onboarding-footer"><span>Profile version {profile?.version ?? "—"}</span><div className="session-buttons"><button className="secondary-button" type="button" disabled={finishing} onClick={() => navigate("/calibration?improve=1")}><Sparkles size={16} />Improve My Profile</button><button className="primary-button" type="button" disabled={finishing || !profile} onClick={() => void finish()}>{finishing ? "Opening…" : "Start Using TracerText"}<ArrowRight size={16} /></button></div></div>
  </section></main>;
}
